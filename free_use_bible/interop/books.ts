...
export async function fetch_books_in_translation(translation: string): Promise<TranslationBooks>
{
    const url = `${BASE_URL}/${translation}/books.json`;
    const data = await fetch_json<TranslationBooks>(url);
    return data;
}