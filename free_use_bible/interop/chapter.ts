...
import { TranslationBook } from "./books";
...
export async function fetch_chapter_in_translation(translation: string, book: string, chapter: number): Promise<TranslationBookChapter>
{
    const url = `${BASE_URL}/${translation}/${book}/${chapter}.json`;
    const data = await fetch_json<TranslationBookChapter>(url);
    return data;
}